"""Payload p08 — monkey-patch ShortConv.forward_cuda decode branch.

Keeps stock prefill path. Decode uses fused B*x + conv_update + C*y when
safe (width==3, state_len==2); otherwise falls back to stock ops inside the
patched method (per-forward fail-open).
"""
...
def _make_forward_cuda(original_forward_cuda):
    ...
    # Keep a handle for debugging / idempotent checks.
    forward_cuda.__develarper_p08__ = True  # type: ignore[attr-defined]
    forward_cuda.__wrapped_original__ = original_forward_cuda  # type: ignore[attr-defined]
    return forward_cuda


@register("p08_shortconv_fuse")
def apply_p08_shortconv_fuse() -> PayloadResult:
    try:
        from vllm.model_executor.layers.mamba.short_conv import ShortConv
    except Exception as exc:
        return PayloadResult(
            name="p08_shortconv_fuse",
            status="skipped",
            reason=f"ShortConv import failed: {exc!r}",
        )

    if getattr(ShortConv.forward_cuda, "__develarper_p08__", False):
        return PayloadResult(
            name="p08_shortconv_fuse",
    ...
    # Class-level probe only. in_proj/out_proj/conv/L_cache are instance attrs
    # created in __init__ — do NOT hasattr() them on the class (false skip).
    if not callable(getattr(ShortConv, "forward_cuda", None)):
        return PayloadResult(
            name="p08_shortconv_fuse",
            status="skipped",
            reason="ShortConv.forward_cuda missing/not callable",
        )
    ...
    return PayloadResult(
        name="p08_shortconv_fuse",
        status="applied",
        reason="ShortConv.forward_cuda patched (decode fuse, prefill stock)",
        details={"width_target": "3", "state_len_target": "2"},
    )
